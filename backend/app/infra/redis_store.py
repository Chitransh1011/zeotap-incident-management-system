...
class RedisStore:
    ...
    async def set_debounce_work_item(self, component_id: str, work_item_id: UUID, ttl_seconds: int) -> None:
        assert self.client
        await self.client.set(f"debounce:{component_id}", str(work_item_id), ex=ttl_seconds)
    ...
    async def upsert_dashboard_item(self, item: dict) -> None:
        ...
        if item["status"] == "CLOSED":
            ...
            return
        await self.client.set(key, json.dumps(item))
    ...
    async def list_dashboard_items(self) -> list[dict]:
        ...
        if not ids:
            return []
        values = await self.client.mget([f"incident:{incident_id}" for incident_id in ids])
        items = [json.loads(value) for value in values if value]
        severity_order = {"P0": 0, "P1": 1, "P2": 2, "P3": 3}
        return sorted(items, key=lambda item: (severity_order.get(item["severity"], 4), item["createdAt"]))
